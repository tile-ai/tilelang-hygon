...
def supply_tensors_gpu(params):
    """Supply function that creates tensors on GPU for HCU."""
    tensors = []
    for param in params:
...
def fast_flashattn(
    ...
    k_pack: int = 2,
    panel_size: int = 7,
    qk_coalesced_width: int = 8,
    v_coalesced_width: int = 4,
):
    scale = (1.0 / dim) ** 0.5
    ...
    def main(
        ...
        with T.Kernel(num_split_q, batch * heads, threads=threads) as (b_split, byz_combined):
            ...
            while bx < num_q_blocks:
                ...
                # P_shared is used to bridge the D-layout (acc_s output) to
                # A-layout (acc_s_cast input for GEMM 2).
                ...
                for k in T.Pipelined(loop_end_k, num_stages=num_stages):
                    ...
                    # D and A have different register layouts.
