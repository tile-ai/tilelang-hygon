...
def matmul(A, B, block_M, block_N, block_K, dtype, accum_dtype=T.float32, num_stages=3):
    M, N, K = T.const("M, N, K")
...
def test_gemm_fp8(M, N, K, dtype):
    ...
    c = cast_gemm_fp8_result(matmul(a, b, bm, bn, bk, dtype, num_stages=stages), dtype)
...
def run_regression_perf():
    ...
    kernel_e4m3 = matmul.compile(M=M, N=N, K=K, block_M=bm, block_N=bn, block_K=bk, dtype=dtype, num_stages=stages)
