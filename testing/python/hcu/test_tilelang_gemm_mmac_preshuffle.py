        ...
        with T.Kernel(T.ceildiv(N, block_N), T.ceildiv(M, block_M), threads=threads) as (bx, by):
            ...
            for ko in T.Pipelined(num_ko, num_stages=0):
                ...
                for ki in T.serial(0, num_ki):
                    ...
                    if b_g2l_load:
                        # Load B G2L
                        mmac_emitter.ldmatrix_b(B_local, B, ki + ko * num_ki, pid_m=by, pid_n=bx)
                    else:
                        # Load B S2L
...
def test_assert_tl_matmul():
    assert_tl_matmul_correctness(256, 256, 256, "int8", "int32", accum_dtype="int32", b_preshuffle=True)
    assert_tl_matmul_correctness(256, 256, 256, "int8", "int32", accum_dtype="int32", b_preshuffle=True)
    assert_tl_matmul_correctness(256, 256, 256, "int8", "int32", b_transposed=False, accum_dtype="int32", b_preshuffle=True)
