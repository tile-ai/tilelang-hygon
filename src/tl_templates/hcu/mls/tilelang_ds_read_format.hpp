...
/*
 * ds_read_format_tensor: read MLS LDS into target buffer with gemm body_rr layout.
 * Indexing: target[(ki * warp_mmac_rows + Mi) * vec_size].
 *
 * warp_mn_idx, warp_k_idx: indices of current warp in the block (caller-provided).
 * target: output buffer, must have at least GemmTensorSize elements (caller-allocated).
 *
 * Each DsFormatInst read yields kMN x kK; we treat it as (kMN/mmacMN) x (kK/mmacK)
 * mmac blocks and scatter them to target positions.
 */
template <typename BlockSize,
          typename MlsTileSize,
          ::tl::index_t WarpMN,
    ...
    ::tl::index_t warp_k_idx)
{
    ...
    ::tl::static_for<0, Traits::NumAccess, 1>{}([&](auto i) {
        ...
        ::tl::static_for<0, NumMMACPerRead, 1>{}([&](auto block_idx) {
            ...
            constexpr auto target_ki =
                k_iter * NumMMAC_K + mmac_k;
            constexpr auto target_mni =
                mn_iter * NumMMAC_MN + mmac_mn;
