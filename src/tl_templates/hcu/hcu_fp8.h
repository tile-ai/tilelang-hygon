...
using fp8_e4_t = tl::fp8_t;
using fp8_e4_2_t = tl::fp8x2_t;
using fp8_e4_4_t = tl::fp8x4_t;
using fp8_e4_8_t = tl::fp8x8_t;
using fp8_e4_16_t = tl::fp8x16_t;

using fp8_e5_t = tl::bf8_t;
using fp8_e5_2_t = tl::bf8x2_t;
using fp8_e5_4_t = tl::bf8x4_t;
using fp8_e5_8_t = tl::bf8x8_t;
using fp8_e5_16_t = tl::bf8x16_t;

// Tilelang's own fp8_e4_t (FP8 E4M3) conversion class
// This provides true conversion with float and supports conversion to tl::fp8_t
struct __align__(1) fp8_cvt_t {
  ...
  TL_DEVICE explicit constexpr operator float() const {
    return tl::fp8_to_float_raw(data);
  }

  // Cast to double - convert through float
  TL_DEVICE explicit constexpr operator double() const {
    return static_cast<double>(tl::fp8_to_float_raw(data));
  }

  // Cast to int
  TL_DEVICE explicit constexpr operator int() const {
    return static_cast<int>(tl::fp8_to_float_raw(data));
  }

  // Conversion to tl::fp8_t
  // This allows seamless interoperability with ck_tile
  TL_DEVICE constexpr operator tl::fp8_t() const {
    return tl::bit_cast<tl::fp8_t>(data);
  }
...
// This provides true conversion with float and supports conversion to tl::bf8_t
struct __align__(1) bf8_cvt_t {
  ...
  // Cast to float - use ck_tile's conversion function
  TL_DEVICE explicit constexpr operator float() const {
    return tl::bf8_to_float_raw(data);
  }

  // Cast to double - convert through float
  TL_DEVICE explicit constexpr operator double() const {
    return static_cast<double>(tl::bf8_to_float_raw(data));
  }

  // Cast to int
  TL_DEVICE explicit constexpr operator int() const {
    return static_cast<int>(tl::bf8_to_float_raw(data));
  }

  // Conversion to tl::bf8_t
  // This allows seamless interoperability with ck_tile
  TL_DEVICE constexpr operator tl::bf8_t() const {
    return tl::bit_cast<tl::bf8_t>(data);
  }
...
TL_DEVICE fp8_e4_4_t make_fp8_e4_4_t(fp8_e4_t x, fp8_e4_t y, fp8_e4_t z,
  ...
  uint8_t z_val = tl::bit_cast<uint8_t>(z);
  uint8_t w_val = tl::bit_cast<uint8_t>(w);
...
TL_DEVICE fp8_e4_8_t make_fp8_e4_8_t(fp8_e4_t x, fp8_e4_t y, fp8_e4_t z,
  ...
  uint8_t z_val = tl::bit_cast<uint8_t>(z);
  uint8_t w_val = tl::bit_cast<uint8_t>(w);
  uint8_t v_val = tl::bit_cast<uint8_t>(v);
  uint8_t u_val = tl::bit_cast<uint8_t>(u);
  uint8_t t_val = tl::bit_cast<uint8_t>(t);
  ...
  // Reinterpret as fp8_e4_8_t (tl::fp8x8_t)
  return tl::bit_cast<fp8_e4_8_t>(packed);
}
...
TL_DEVICE fp8_e4_16_t make_fp8_e4_16_t(fp8_e4_t x, fp8_e4_t y, fp8_e4_t z,
  ...
  uint8_t t_val = tl::bit_cast<uint8_t>(t);
  uint8_t s_val = tl::bit_cast<uint8_t>(s);
  uint8_t r_val = tl::bit_cast<uint8_t>(r);
  uint8_t q_val = tl::bit_cast<uint8_t>(q);
  uint8_t p_val = tl::bit_cast<uint8_t>(p);
  uint8_t o_val = tl::bit_cast<uint8_t>(o);
  uint8_t n_val = tl::bit_cast<uint8_t>(n);
  uint8_t m_val = tl::bit_cast<uint8_t>(m);
  ...
  // Reinterpret as fp8_e4_16_t (tl::fp8x16_t)
  return tl::bit_cast<fp8_e4_16_t>(packed);
}
