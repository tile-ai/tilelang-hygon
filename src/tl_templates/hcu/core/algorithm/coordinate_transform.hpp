...
{
    TL_HOST_DEVICE static constexpr auto get_type_enum()
    {
        return coord_transform_enum::undefined;
    }

    TL_HOST_DEVICE static constexpr index_t get_num_of_lower_dimension() { return NDimLow; }

    TL_HOST_DEVICE static constexpr index_t get_num_of_upper_dimension() { return NDimUp; }
    ...
    TL_HOST_DEVICE static constexpr auto
...
struct pass_through : public base_transform<1, 1>
{
    ...
    TL_HOST_DEVICE constexpr pass_through() = default;

    TL_HOST_DEVICE constexpr pass_through(const LowLength& low_length)
    ...
    TL_HOST_DEVICE constexpr const auto& get_upper_lengths() const { return up_lengths_; }

    template <typename LowIdx, typename UpIdx>
    TL_HOST_DEVICE static constexpr void calculate_lower_index(LowIdx& idx_low,
                                                                    const UpIdx& idx_up)
    {
    ...
    TL_HOST_DEVICE static void update_lower_index(LowIdxDiff& idx_diff_low,
    ...
    TL_HOST_DEVICE static constexpr bool
    ...
    TL_HOST_DEVICE static constexpr bool is_known_at_compile_time()
    {
        return tl::is_known_at_compile_time<UpLengths>::value;
    }
...
struct pad : public base_transform<1, 1>
{
    ...
    TL_HOST_DEVICE constexpr pad() : up_lengths_{}, left_pad_length_{}, right_pad_length_{} {}

    TL_HOST_DEVICE constexpr pad(const LowLength& low_length,
    ...
    template <typename LowIdx, typename UpIdx>
    TL_HOST_DEVICE constexpr void calculate_lower_index(LowIdx& idx_low,
    ...
    {
        return tl::is_known_at_compile_time<UpLengths>::value &&
               tl::is_known_at_compile_time<LeftPadLength>::value &&
               tl::is_known_at_compile_time<RightPadLength>::value;
    }
...
struct left_pad
{
    ...
    TL_HOST_DEVICE constexpr left_pad() = default;

    TL_HOST_DEVICE constexpr left_pad(const LowLength& low_length,
                                           const LeftPadLength& left_pad_length)
...
struct right_pad : public base_transform<1, 1>
{
    ...
    TL_HOST_DEVICE constexpr right_pad() = default;

    TL_HOST_DEVICE constexpr right_pad(const LowLength& low_length,
                                            const RightPadLength& right_pad_length)
    ...
    {
        return tl::is_known_at_compile_time<UpLengths>::value &&
               tl::is_known_at_compile_time<LowLength>::value &&
               tl::is_known_at_compile_time<RightPadLength>::value;
    }
...
struct embed : public base_transform<1, UpLengths::size()>
{
    ...
    TL_HOST_DEVICE constexpr embed() = default;

    TL_HOST_DEVICE constexpr embed(const UpLengths& up_lengths,
    ...
    template <typename LowIdxDiff, typename UpIdxDiff, typename LowIdx, typename UpIdx>
    TL_HOST_DEVICE void update_lower_index(LowIdxDiff& idx_diff_low,
                                                const UpIdxDiff& idx_diff_up,
                                                LowIdx& idx_low,
                                                const UpIdx&) const
    {
    ...
    {
        return tl::is_known_at_compile_time<UpLengths>::value &&
               tl::is_known_at_compile_time<Coefficients>::value;
    }
...
{
    template <index_t I>
    TL_HOST_DEVICE constexpr auto operator()(number<I> i) const
    {
...
struct merge_v2_magic_division : public base_transform<LowLengths::size(), 1>
{
    ...
    TL_HOST_DEVICE constexpr merge_v2_magic_division() = default;

    TL_HOST_DEVICE constexpr merge_v2_magic_division(const LowLengths& low_lengths)
    ...
    TL_HOST_DEVICE static constexpr bool is_known_at_compile_time()
    {
        return tl::is_known_at_compile_time<LowLengths>::value &&
               tl::is_known_at_compile_time<LowLengthsMagicDivisor>::value &&
               tl::is_known_at_compile_time<UpLengths>::value;
    }
...
struct merge_v3_division_mod : public base_transform<LowLengths::size(), 1>
{
    ...
    TL_HOST_DEVICE constexpr merge_v3_division_mod() = default;

    TL_HOST_DEVICE constexpr merge_v3_division_mod(const LowLengths& low_lengths)
        : low_lengths_{low_lengths},
          low_lengths_scan_{
    ...
    {
        return tl::is_known_at_compile_time<LowLengths>::value &&
               tl::is_known_at_compile_time<LowLengthsScan>::value &&
               tl::is_known_at_compile_time<UpLengths>::value;
    }
...
struct unmerge : public base_transform<1, UpLengths::size()>
{
    ...
    TL_HOST_DEVICE constexpr unmerge() = default;

    TL_HOST_DEVICE constexpr unmerge(const UpLengths& up_lengths)
    ...
    {
        return tl::is_known_at_compile_time<UpLengths>::value &&
               tl::is_known_at_compile_time<UpLengthsScan>::value;
    }
                                                         ...
                                                         const LowVectorStrides& low_vector_strides)
    {
        ...
        if constexpr(tl::is_known_at_compile_time<decltype(up_length_last)>::value)
        {
...
struct freeze : public base_transform<1, 0>
{
    ...
    TL_HOST_DEVICE constexpr freeze() = default;

    TL_HOST_DEVICE constexpr freeze(const LowerIndex& low_idx) : low_idx_{low_idx} {}

    TL_HOST_DEVICE static constexpr auto get_upper_lengths() { return tuple<>{}; }
    ...
    TL_HOST_DEVICE static constexpr bool is_known_at_compile_time()
    {
        return tl::is_known_at_compile_time<LowerIndex>::value;
    }
...
struct insert : public base_transform<0, 1>
{
    ...
    TL_HOST_DEVICE constexpr insert() = default;

    TL_HOST_DEVICE constexpr insert(const UpperLength& up_length)
        : up_lengths_{make_tuple(up_length)}
    {
    }

    TL_HOST_DEVICE static constexpr index_t get_num_of_lower_dimension() { return 0; }

    TL_HOST_DEVICE static constexpr index_t get_num_of_upper_dimension() { return 1; }

    TL_HOST_DEVICE constexpr auto get_upper_lengths() const { return up_lengths_; }

    template <typename LowIdx, typename UpIdx>
    TL_HOST_DEVICE constexpr void calculate_lower_index(LowIdx&, const UpIdx&) const
    {
    ...
    TL_HOST_DEVICE static constexpr bool IsLinearTransform() { return true; }
    ...
    TL_HOST_DEVICE static constexpr bool is_known_at_compile_time()
    {
        return tl::is_known_at_compile_time<UpperLength>::value;
    }
...
struct replicate : public base_transform<0, UpLengths::size()>
{
    ...
    TL_HOST_DEVICE constexpr replicate() = default;

    TL_HOST_DEVICE constexpr replicate(const UpLengths& up_lengths) : up_lengths_{up_lengths}
    {
    }
...
struct slice : public base_transform<1, 1>
{
    ...
    TL_HOST_DEVICE constexpr slice() = default;

    TL_HOST_DEVICE constexpr slice(const LowLength&,
    ...
    {
        return tl::is_known_at_compile_time<UpLengths>::value &&
               tl::is_known_at_compile_time<SliceBegin>::value &&
               tl::is_known_at_compile_time<SliceEnd>::value;
    }
...
struct modulo : public base_transform<1, 1>
{
    ...
    TL_HOST_DEVICE constexpr modulo() = default;

    TL_HOST_DEVICE constexpr modulo(const Modulus& modulus, const UpLength& up_length)
...
struct xor_t : public base_transform<2, 2>
{
    ...
    TL_HOST_DEVICE constexpr xor_t() : up_lengths_{} {}

    TL_HOST_DEVICE constexpr xor_t(const LowLengths& low_lengths) : up_lengths_{low_lengths} {}
    ...
    // MUST be static function
    template <typename LowVectorLengths, typename LowVectorStrides>
    TL_HOST_DEVICE constexpr auto calculate_upper_dimension_safe_vector_length_strides(
...
struct offset : public base_transform<1, 1>
{
    ...
    TL_HOST_DEVICE constexpr offset() = default;

    TL_HOST_DEVICE constexpr offset(const LowLength& low_length,
    ...
    {
        return tl::is_known_at_compile_time<UpLengths>::value &&
               tl::is_known_at_compile_time<OffsetLength>::value;
    }
...
struct indexing : public base_transform<1, 1>
{
    ...
    TL_HOST_DEVICE constexpr indexing() = default;

    TL_HOST_DEVICE constexpr indexing(const UpLength& up_length,
...
template <typename LowLength>
TL_HOST_DEVICE constexpr auto make_pass_through_transform(const LowLength& low_length)
{
...
template <typename UpLengths,
          typename Coefficients,
          typename std::enable_if<UpLengths::size() == Coefficients::size(), bool>::type = false>
TL_HOST_DEVICE constexpr auto make_embed_transform(const UpLengths& up_lengths,
                                                        const Coefficients& coefficients)
{
...
template <typename LowLengths>
TL_HOST_DEVICE constexpr auto make_merge_transform(const LowLengths& low_lengths)
{
...
template <typename LowerIndex>
TL_HOST_DEVICE constexpr auto make_freeze_transform(const LowerIndex& low_idx)
{
    return freeze<LowerIndex>{low_idx};
}

template <typename UpperIndex>
TL_HOST_DEVICE constexpr auto make_insert_transform(const UpperIndex& up_idx)
{
    return insert<UpperIndex>{up_idx};
}

template <typename UpLengths>
TL_HOST_DEVICE constexpr auto make_replicate_transform(const UpLengths& up_lengths)
{
    return replicate<UpLengths>{up_lengths};
}

template <typename LowLength, typename SliceBegin, typename SliceEnd>
TL_HOST_DEVICE constexpr auto make_slice_transform(const LowLength& low_length,
                                                        const SliceBegin& slice_begin,
                                                        const SliceEnd& slice_end)
{
...
template <typename Modulus, typename UpLength>
TL_HOST_DEVICE constexpr auto make_modulo_transform(const Modulus& modulus,
...
template <typename LowLengths>
TL_HOST_DEVICE constexpr auto make_xor_transform(const LowLengths& low_lengths)
{
    return xor_t<LowLengths>{low_lengths};
}

template <typename LowLength, typename OffsetLength>
TL_HOST_DEVICE constexpr auto make_offset_transform(const LowLength& low_length,
                                                         const OffsetLength& offset_length)
{
...
TL_HOST_DEVICE constexpr auto make_indexing_transform(const UpLength& up_lengths,
