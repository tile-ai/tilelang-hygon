...
{
    TL_HOST_DEVICE uint32_t operator()(int, T, uint32_t = seed_) { return 0; }
};
...
{
    TL_HOST_DEVICE uint32_t operator()(int id, float val, uint32_t seed = seed_)
    {
...
{
    TL_HOST_DEVICE uint32_t operator()(int id, half_t val, uint32_t seed = seed_)
    {
