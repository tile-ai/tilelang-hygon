...
TL_HOST_DEVICE constexpr auto make_multi_index(Xs&&... xs)
{
    return make_array<index_t>(index_t{xs}...);
}

template <index_t NSize>
TL_HOST_DEVICE constexpr auto make_zero_multi_index()
{
...
template <typename T>
TL_HOST_DEVICE constexpr auto to_multi_index(const T& x)
{
    return unpack([](auto... ys) { return make_multi_index(ys...); }, x);
}

template <index_t NSize, typename X>
TL_HOST_DEVICE constexpr auto operator+=(multi_index<NSize>& y, const X& x)
{
...
TL_HOST_DEVICE constexpr auto operator+(const multi_index<NSize>& a, const T& b)
{
...
template <index_t NSize>
TL_HOST_DEVICE constexpr auto operator*(index_t a, const multi_index<NSize>& x)
{
    multi_index<NSize> r;
...
template <index_t NSize>
TL_HOST_DEVICE constexpr auto operator*(const multi_index<NSize>& x, index_t a)
{
    return a * x;
}
