...
TL_HOST_DEVICE constexpr auto sequence_pop_front(sequence<I, Is...>);

template <typename Seq>
TL_HOST_DEVICE constexpr auto sequence_pop_back(Seq);
...
struct sequence
{
    ...
    TL_HOST_DEVICE static constexpr index_t size() { return sizeof...(Is); }
    TL_HOST_DEVICE static constexpr bool is_static() { return true; };

    template <index_t I>
    TL_HOST_DEVICE static constexpr auto get()
    {
    ...
    template <index_t I>
    TL_HOST_DEVICE static constexpr auto get(number<I>)
    {
        static_assert(I < size(), "wrong! I too large");
        return number<get<I>()>{};
    }

    TL_HOST_DEVICE static constexpr index_t at(index_t I)
    {
    ...
    template <index_t I>
    TL_HOST_DEVICE static constexpr auto at()
    {
        static_assert(I < size(), "wrong! I too large");
        return number<impl::at_index_t<I, constant<Is>...>{}>{};
    }
    ...
    template <index_t I>
    TL_HOST_DEVICE static constexpr auto at(number<I>)
    {
        static_assert(I < size(), "wrong! I too large");
        return number<get<I>()>{};
    }
    ...
    template <typename I>
    TL_HOST_DEVICE constexpr auto operator[](I i) const
    {
        return at(i);
    }

    template <index_t... IRs>
    TL_HOST_DEVICE static constexpr auto reorder_new_to_old(sequence<IRs...> /*new2old*/)
    {
    ...
    TL_HOST_DEVICE static constexpr auto reorder_old_to_new(MapOld2New)
    {
    ...
    TL_HOST_DEVICE static constexpr auto reverse()
    {
        return typename sequence_reverse<type>::type{};
    }

    TL_HOST_DEVICE static constexpr auto front()
    {
        static_assert(size() > 0, "wrong!");
        return get(number<0>{});
    }

    TL_HOST_DEVICE static constexpr auto back()
    {
        static_assert(size() > 0, "wrong!");
        return get(number<size() - 1>{});
    }

    TL_HOST_DEVICE static constexpr auto pop_front() { return sequence_pop_front(type{}); }

    TL_HOST_DEVICE static constexpr auto pop_back() { return sequence_pop_back(type{}); }

    template <index_t... Xs>
    TL_HOST_DEVICE static constexpr auto push_front(sequence<Xs...>)
    {
        return sequence<Xs..., Is...>{};
    }

    template <index_t... Xs>
    TL_HOST_DEVICE static constexpr auto push_front(number<Xs>...)
    {
        return sequence<Xs..., Is...>{};
    }

    template <index_t... Xs>
    TL_HOST_DEVICE static constexpr auto push_back(sequence<Xs...>)
    {
        return sequence<Is..., Xs...>{};
    }

    template <index_t... Xs>
    TL_HOST_DEVICE static constexpr auto push_back(number<Xs>...)
    {
        return sequence<Is..., Xs...>{};
    }
    ...
    TL_HOST_DEVICE static constexpr auto extract(number<Ids>...)
    {
        return sequence<type::get(number<Ids>{})...>{};
    }

    template <index_t... Ids>
    TL_HOST_DEVICE static constexpr auto extract(sequence<Ids...>)
    {
        return sequence<type::get(number<Ids>{})...>{};
    }
    ...
    TL_HOST_DEVICE static constexpr auto modify(number<I>, number<X>)
    {
    ...
    template <typename F>
    TL_HOST_DEVICE static constexpr auto transform(F f)
    {
        return sequence<f(Is)...>{};
    }

    TL_HOST_DEVICE static void print()
    {
...
struct arithmetic_sequence_gen
{
    struct F
    {
        TL_HOST_DEVICE constexpr index_t operator()(index_t i) const
        {
...
struct uniform_sequence_gen
{
    struct F
    {
        TL_HOST_DEVICE constexpr index_t operator()(index_t) const { return I; }
    };
...
template <index_t... Xs, index_t... Ys>
TL_HOST_DEVICE constexpr bool operator==(sequence<Xs...>, sequence<Ys...>)
{
    return ((Xs == Ys) && ...);
}

template <index_t... Xs, index_t... Ys>
TL_HOST_DEVICE constexpr bool operator!=(sequence<Xs...> x, sequence<Ys...> y)
{
    return !(x == y);
}

template <index_t... Xs, index_t... Ys>
TL_HOST_DEVICE constexpr auto operator+(sequence<Xs...>, sequence<Ys...>)
{
...
TL_HOST_DEVICE constexpr auto operator+(sequence<Xs...>, number<Y>)
{
...
TL_HOST_DEVICE constexpr auto operator+(number<Y>, sequence<Xs...>)
{
...
TL_HOST_DEVICE constexpr auto merge_sequences(Seqs...)
{
    return typename sequence_merge<Seqs...>::type{};
}

template <typename F, index_t... Xs>
TL_HOST_DEVICE constexpr auto transform_sequences(F f, sequence<Xs...>)
{
    return sequence<f(Xs)...>{};
}

template <typename F, index_t... Xs, index_t... Ys>
TL_HOST_DEVICE constexpr auto transform_sequences(F f, sequence<Xs...>, sequence<Ys...>)
{
...
TL_HOST_DEVICE constexpr auto reverse_inclusive_scan_sequence(Seq, Reduce, number<Init>)
{
    return typename sequence_reverse_inclusive_scan<Seq, Reduce, Init>::type{};
}

template <typename Seq, typename Reduce, index_t Init>
TL_HOST_DEVICE constexpr auto reverse_exclusive_scan_sequence(Seq, Reduce, number<Init>)
{
...
template <typename Seq, typename Reduce, index_t Init>
TL_HOST_DEVICE constexpr auto inclusive_scan_sequence(Seq, Reduce, number<Init>)
{
...
TL_HOST_DEVICE constexpr auto pick_sequence_elements_by_ids(Seq, sequence<Is...> /* ids */)
{
...
TL_HOST_DEVICE constexpr auto pick_sequence_elements_by_mask(Seq, Mask)
{
...
TL_HOST_DEVICE constexpr auto modify_sequence_elements_by_ids(Seq, Values, Ids)
{
...
template <typename Seq, typename Reduce, index_t Init>
TL_HOST_DEVICE constexpr index_t
...
TL_HOST_DEVICE constexpr bool sequence_any_of(Seq, F f)
{
    bool flag = false;
...
template <typename Seq, typename F>
TL_HOST_DEVICE constexpr bool sequence_all_of(Seq, F f)
{
    bool flag = true;
...
template <index_t... Is>
TL_HOST_DEVICE constexpr auto make_sequence(number<Is>...)
{
    return sequence<Is...>{};
}
...
TL_HOST_DEVICE constexpr auto generate_sequence(F, number<N>)
{
...
template <typename F, index_t N>
TL_HOST_DEVICE constexpr auto generate_sequence_v2(F&& f, number<N>)
{
...
template <index_t... Is>
TL_HOST_DEVICE constexpr auto to_sequence(tuple<number<Is>...>)
{
    return sequence<Is...>{};
}
...
TL_HOST_DEVICE constexpr auto histogram_sorted_sequence(SeqSortedSamples, sequence<r, rs...>)
{
...
template <typename F, index_t N>
TL_HOST_DEVICE constexpr auto generate_array(F&& f, number<N>)
{
