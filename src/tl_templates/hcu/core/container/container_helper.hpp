...
TL_HOST_DEVICE constexpr auto container_push_back(const array<TData, NSize>& a, const TData& x)
{
...
TL_HOST_DEVICE constexpr auto container_push_front(const tuple<Ts...>& a, const T& x)
{
    return container_concat(make_tuple(x), a);
}

template <typename... Ts, typename T>
TL_HOST_DEVICE constexpr auto container_push_back(const tuple<Ts...>& a, const T& x)
{
...
TL_HOST_DEVICE constexpr auto container_reorder_given_new2old(const tuple<Ts...>& old_tuple,
...
template <typename... Ts, index_t... IRs>
TL_HOST_DEVICE constexpr auto container_reorder_given_old2new(const tuple<Ts...>& old_tuple,
...
TL_HOST_DEVICE constexpr auto container_reorder_given_new2old(sequence<Is...> /* old_seq */,
                                                                   sequence<IRs...> /*new2old*/)
{
...
template <index_t... Is, index_t... IRs>
TL_HOST_DEVICE constexpr auto container_reorder_given_old2new(sequence<Is...> old_seq,
                                                                   sequence<IRs...> /* old2new */)
{
    static_assert(sizeof...(Is) == sizeof...(IRs), "wrong! size not consistent");
...
template <typename Container,
          ...
          index_t IStep  = 1>
TL_HOST_DEVICE constexpr auto container_reduce(const Container& x,
                                                    Reduce reduce,
                                                    Init init,
...
template <typename Container,
          ...
          index_t IEnd,
          index_t IStep>
TL_HOST_DEVICE constexpr auto container_reduce_impl(
...
TL_HOST_DEVICE constexpr auto container_reverse_exclusive_scan_impl(
...
TL_HOST_DEVICE constexpr auto container_concat(const X& x, const Ys&... ys)
{
    return container_concat(x, container_concat(ys...));
}

template <typename T, index_t NX, index_t NY>
TL_HOST_DEVICE constexpr auto container_concat(const array<T, NX>& ax, const array<T, NY>& ay)
{
    return unpack2(
...
TL_HOST_DEVICE constexpr auto container_concat(const tuple<X...>& tx, const tuple<Y...>& ty)
{
    return unpack2(
...
template <typename Container>
TL_HOST_DEVICE constexpr auto container_concat(const Container& x)
{
    return x;
}

template <typename T, index_t N, index_t... Is>
TL_HOST_DEVICE constexpr auto get_container_subset(const array<T, N>& arr, sequence<Is...>)
{
...
TL_HOST_DEVICE constexpr auto get_container_subset(const tuple<Ts...>& tup, sequence<Is...>)
{
...
TL_HOST_DEVICE constexpr void set_container_subset(Y& y, sequence<Is...> picks, const X& x)
{
...
template <index_t... Is>
TL_HOST_DEVICE constexpr auto sequence_to_tuple_of_number(sequence<Is...>)
{
