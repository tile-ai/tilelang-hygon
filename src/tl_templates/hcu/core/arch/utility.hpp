...
TL_DEVICE void m0_set_with_memory(index_t v)
{
    asm volatile("s_mov_b32 m0, %0" : : "s"(v) : "memory");
}

// NOTE: this is an immediate value
TL_DEVICE void m0_inc_with_memory(index_t v)
{
    asm volatile("s_add_u32 m0, %0, m0" : : "n"(v) : "memory");
}

template <typename T>
TL_DEVICE T warp_shuffle_up(const T& v_local, uint32_t lane_delta)
{
#if 0
...
template <typename T>
TL_DEVICE T warp_shuffle_down(const T& v_local, uint32_t lane_delta)
{
#if 0
...
template <typename T>
TL_DEVICE T warp_shuffle(const T& v_local, uint32_t src_lane)
{
#if 0
