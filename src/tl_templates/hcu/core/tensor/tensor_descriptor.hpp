                                                 ...
                                                 TopDimensionHiddenIds>
{
    ...
    public:
    TL_HOST_DEVICE constexpr tensor_descriptor() = default;

    TL_HOST_DEVICE constexpr tensor_descriptor(const Transforms& transforms,
    ...
    TL_HOST_DEVICE constexpr tensor_descriptor(const Base& adaptor,
                                                    ElementSpaceSize element_space_size)
        : Base{adaptor}, element_space_size_{element_space_size}
    {
    }

    TL_HOST_DEVICE static constexpr index_t get_num_of_dimension()
    {
        return Base::get_num_of_top_dimension();
    }

    template <index_t IDim>
    TL_HOST_DEVICE constexpr auto get_length(number<IDim> idim) const
    {
        return Base::get_top_dimension_length(idim);
    }

    TL_HOST_DEVICE constexpr auto get_lengths() const
    {
        return Base::get_top_dimension_lengths();
    }

    TL_HOST_DEVICE constexpr auto get_element_space_size() const
    {
        return element_space_size_;
    }

    template <typename Idx>
    TL_HOST_DEVICE constexpr index_t calculate_offset(const Idx& idx) const
    {
        return Base::calculate_bottom_index(idx)[number<0>{}];
    }

    // TODO make these private
    TL_HOST_DEVICE constexpr const auto& get_transforms() const
    {
    ...
    TL_HOST_DEVICE static constexpr bool is_static()
    {
        return Base::is_known_at_compile_time() &&
               tl::is_known_at_compile_time<ElementSpaceSize>::value;
    }
...
TL_HOST_DEVICE constexpr auto calculate_element_space_size_impl(const Lengths& lengths,
...
template <typename... Lengths,
          ...
          index_t GuaranteedLastDimensionVectorLength                                   = -1,
          typename std::enable_if<sizeof...(Lengths) == sizeof...(Strides), bool>::type = false>
TL_HOST_DEVICE constexpr auto make_naive_tensor_descriptor_packed_with_offset(
