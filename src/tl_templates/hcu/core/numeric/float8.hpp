...
#define TL_FP8_CVT_DEVICE 1
#else
#define TL_FP8_CVT_DEVICE 0
#endif
...
template <fp8_rounding_mode rounding = static_cast<fp8_rounding_mode>(TL_FLOAT_TO_FP8_DEFAULT)>
TL_HOST_DEVICE uint8_t float_to_fp8_raw(float, constant<rounding> = {});

template <fp8_rounding_mode rounding = static_cast<fp8_rounding_mode>(TL_FLOAT_TO_FP8_DEFAULT)>
TL_HOST_DEVICE uint8_t float_to_bf8_raw(float, constant<rounding> = {});

TL_HOST_DEVICE float fp8_to_float_raw(uint8_t);
TL_HOST_DEVICE float bf8_to_float_raw(uint8_t);
...
TL_HOST_DEVICE DstT run_cast_to_f8(SrcT src, unsigned int rng = 0)
{
...
TL_HOST_DEVICE DstT run_cast_from_f8(SrcT x)
{
...
TL_HOST_DEVICE Y cast_to_f8(X x, uint32_t rng)
{
    return bit_cast<Y>(run_cast_to_f8<X, Y, clip, stoch>(x, rng));
}

#if TL_FP8_CVT_DEVICE
/**
...
TL_DEVICE uint8_t cast_to_f8_from_f32(float v, unsigned int rng = 0)
{
    uint8_t i8data;
    union
    {
...
TL_HOST_DEVICE typename numeric_traits<DstT>::bitwise_type float_to_fp8_sr_raw(SrcT x)
{
 ...
 */
template <typename SrcT, typename DstT>
TL_HOST_DEVICE typename numeric_traits<DstT>::bitwise_type float_to_fp8_rtn_raw(SrcT x)
{
    constexpr bool clip = true;
#if TL_FP8_CVT_DEVICE
...
TL_HOST_DEVICE fp8_raw_t float_to_fp8_raw(float x, constant<rounding>)
{
...
template <fp8_rounding_mode rounding>
TL_HOST_DEVICE bf8_raw_t float_to_bf8_raw(float x, constant<rounding>)
{
    if constexpr(rounding == fp8_rounding_mode::standard)
    {
...
TL_HOST_DEVICE float fp8_to_float_raw(fp8_raw_t x)
{
#if TL_FP8_CVT_DEVICE
    float fval;
...
TL_HOST_DEVICE float bf8_to_float_raw(bf8_raw_t x)
{
#if TL_FP8_CVT_DEVICE
    float fval;
    uint32_t i32val = static_cast<uint32_t>(x);
...
template <fp8_rounding_mode rounding = static_cast<fp8_rounding_mode>(TL_FLOAT_TO_FP8_DEFAULT)>
TL_HOST_DEVICE fp8_t float_to_fp8(float x, constant<rounding> = {})
{
    return bit_cast<fp8_t>(float_to_fp8_raw(x, constant<rounding>{}));
}

template <fp8_rounding_mode rounding = static_cast<fp8_rounding_mode>(TL_FLOAT_TO_FP8_DEFAULT)>
TL_HOST_DEVICE bf8_t float_to_bf8(float x, constant<rounding> = {})
{
    return bit_cast<bf8_t>(float_to_bf8_raw(x, constant<rounding>{}));
}

TL_HOST_DEVICE float fp8_to_float(fp8_t x) { return fp8_to_float_raw(bit_cast<fp8_raw_t>(x)); }

TL_HOST_DEVICE float bf8_to_float(bf8_t x) { return bf8_to_float_raw(bit_cast<bf8_raw_t>(x)); }
...
struct numeric<fp8_t>
{
    ...
    // returns fp8_t(0.125)
    TL_HOST_DEVICE static constexpr fp8_t epsilon()
    {
    ...
    // half of epsilon
    TL_HOST_DEVICE static constexpr fp8_t round_error()
    {
        return bit_cast<fp8_t>(static_cast<fp8_raw_t>(0x18)); // 0.0625
    }

    // quiet NaN
    TL_HOST_DEVICE static constexpr fp8_t quiet_NaN()
    {
        return bit_cast<fp8_t>(static_cast<fp8_raw_t>(0x7F)); // 0b01111111
    }

    // signaling NaN
    TL_HOST_DEVICE static constexpr fp8_t signaling_NaN()
    {
        return bit_cast<fp8_t>(static_cast<fp8_raw_t>(0xFF)); // 0b11111111
    }

    // smallest positive subnormal value
    TL_HOST_DEVICE static constexpr fp8_t denorm_min()
    {
        return bit_cast<fp8_t>(static_cast<fp8_raw_t>(0x01));
    }

    TL_HOST_DEVICE static constexpr fp8_t zero()
    {
...
struct numeric<bf8_t>
{
    ...
    TL_HOST_DEVICE static constexpr bf8_t epsilon()
    {
    ...
    // half of epsilon
    TL_HOST_DEVICE static constexpr bf8_t round_error()
    {
        return bit_cast<bf8_t>(static_cast<bf8_raw_t>(0x30)); // 0.125
    }

    // positive infinity value
    TL_HOST_DEVICE static constexpr bf8_t infinity()
    {
        return bit_cast<bf8_t>(static_cast<bf8_raw_t>(0x7c)); // 0b01111100
    }

    // quiet NaN
    TL_HOST_DEVICE static constexpr bf8_t quiet_NaN()
    {
        return bit_cast<bf8_t>(static_cast<bf8_raw_t>(0x7F)); // 0b01111111
    }

    // signaling NaN
    TL_HOST_DEVICE static constexpr bf8_t signaling_NaN()
    {
        return bit_cast<bf8_t>(static_cast<bf8_raw_t>(0xFF));
    }

    // smallest positive subnormal value
    TL_HOST_DEVICE static constexpr bf8_t denorm_min()
    {
        return bit_cast<bf8_t>(static_cast<bf8_raw_t>(0x01));
    }

    TL_HOST_DEVICE static constexpr bf8_t zero()
    {
...
struct numeric<fp8_t>
{
    ...
    // positive infinity value
    TL_HOST_DEVICE static constexpr fp8_t infinity()
    {
...
#if TL_USE_CUSTOM_DATA_TYPE
TL_ARITHMETIC_USING_FLOAT(TL_HOST_DEVICE, fp8_t)
TL_ARITHMETIC_USING_FLOAT(TL_HOST_DEVICE, bf8_t)
#endif

// math
template <typename T>
TL_HOST_DEVICE T abs(const T& x)
{
