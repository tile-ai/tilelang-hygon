...
#define TL_FP4_CVT_DEVICE 1
#else
#define TL_FP4_CVT_DEVICE 0
#endif
...
TL_HOST_DEVICE constexpr uint8_t float_to_e2m1(float);
...
struct pk_float4_e2m1_t
{
    ...
    TL_HOST_DEVICE constexpr pk_float4_e2m1_t() : data{type{}} {}
    template <typename T, typename = std::enable_if_t<std::is_integral_v<T>>>
    TL_HOST_DEVICE constexpr pk_float4_e2m1_t(T init) : data{static_cast<type>(init)}
    {
    }
    TL_HOST_DEVICE explicit constexpr pk_float4_e2m1_t(float init) : data{float_to_e2m1(init)}
    {
    }
    TL_HOST_DEVICE constexpr operator type() const { return data; }
    TL_HOST_DEVICE constexpr raw_type& get() { return data; }
    TL_HOST_DEVICE constexpr raw_type get() const { return data; }
    TL_HOST_DEVICE constexpr operator float() const;
    TL_HOST_DEVICE constexpr operator fp32x2_t() const;
    TL_HOST_DEVICE constexpr operator fp16_t() const;
    TL_HOST_DEVICE constexpr operator fp16x2_t() const;
    TL_HOST_DEVICE constexpr operator bf16_t() const;
    TL_HOST_DEVICE constexpr operator bf16x2_t() const;

    template <index_t I>
    TL_HOST_DEVICE constexpr raw_type unpack(number<I>) const;
    TL_HOST_DEVICE constexpr static pk_float4_e2m1_t pack(const type x0, const type x1)
    {
...
struct numeric<pk_fp4_t>
{
    ...
    TL_HOST_DEVICE static constexpr pk_fp4_t min() { return binary_min_normal; }
    TL_HOST_DEVICE static constexpr pk_fp4_t max() { return binary_max_normal; }
    TL_HOST_DEVICE static constexpr pk_fp4_t lowest() { return binary_lowest_normal; }
    TL_HOST_DEVICE static constexpr pk_fp4_t epsilon() { return binary_min_subnorm; }
    TL_HOST_DEVICE static constexpr pk_fp4_t round_error() { return binary_min_subnorm; }
    TL_HOST_DEVICE static constexpr pk_fp4_t zero() { return binary_zero; }
    TL_HOST_DEVICE static constexpr fp8_t denorm_min() { return binary_min_subnorm; }

    TL_HOST_DEVICE static constexpr bool has_inf() { return false; }
    // N/A
    TL_HOST_DEVICE static constexpr pk_fp4_t infinity() { return max(); }
    // N/A
    TL_HOST_DEVICE static constexpr pk_fp4_t quiet_NaN() { return max(); }
    // N/A
    TL_HOST_DEVICE static constexpr pk_fp4_t signaling_NaN() { return max(); }
};

template <index_t I>
TL_HOST_DEVICE constexpr pk_fp4_raw_t pk_fp4_t::unpack(number<I>) const
{
...
TL_ARITHMETIC_USING_FLOAT(TL_HOST_DEVICE, pk_fp4_t)
// TODO: consider replace this macro to improve performance

#if TL_FP4_CVT_DEVICE
namespace impl {

template <typename T>
TL_DEVICE T _from_f4(pk_fp4_raw_t src, float scale = 1.0f)
{
...
template <typename T>
TL_DEVICE pk_fp4_raw_t _to_f4(T src, float scale = 1.0f)
{
    union
    {
        uint32_t u32;
...
TL_HOST_DEVICE constexpr pk_fp4_t::operator bf16_t() const
{
#if TL_FP4_CVT_DEVICE
...
TL_HOST_DEVICE constexpr pk_fp4_t::operator bf16x2_t() const
{
#if TL_FP4_CVT_DEVICE
...
TL_HOST_DEVICE constexpr pk_fp4_raw_t float_to_e2m1(float x)
{
#if TL_FP4_CVT_DEVICE
...
TL_HOST_DEVICE constexpr fp32x2_t pk_fp4_to_fp32x2(const pk_fp4_t& x) { return fp32x2_t(x); }
TL_HOST_DEVICE constexpr fp16x2_t pk_fp4_to_fp16x2(const pk_fp4_t& x) { return fp16x2_t(x); }
TL_HOST_DEVICE constexpr bf16x2_t pk_fp4_to_bf16x2(const pk_fp4_t& x) { return bf16x2_t(x); }
TL_HOST_DEVICE constexpr pk_fp4_t float_to_pk_fp4(const float& x) { return float_to_e2m1(x); }
TL_HOST_DEVICE constexpr pk_fp4_t fp16_to_pk_fp4(const fp16_t& x)
{
#if TL_FP4_CVT_DEVICE
    return impl::_to_f4(x);
#else
...
TL_HOST_DEVICE constexpr pk_fp4_t bf16_to_pk_fp4(const bf16_t& x)
{
#if TL_FP4_CVT_DEVICE
    return impl::_to_f4(x);
#else
...
TL_HOST_DEVICE constexpr pk_fp4_t fp16x2_to_pk_fp4(const fp16x2_t& x)
{
#if TL_FP4_CVT_DEVICE
    return impl::_to_f4(x);
#else
...
TL_HOST_DEVICE constexpr pk_fp4_t bf16x2_to_pk_fp4(const bf16x2_t& x)
{
#if TL_FP4_CVT_DEVICE
    return impl::_to_f4(x);
#else
...
TL_HOST_DEVICE constexpr pk_fp4_t fp32x2_to_pk_fp4(const fp32x2_t& x)
{
#if TL_FP4_CVT_DEVICE
    return impl::_to_f4(x);
#else
...
TL_HOST_DEVICE constexpr pk_fp4_t::operator float() const
{
#if TL_FP4_CVT_DEVICE
...
TL_HOST_DEVICE constexpr pk_fp4_t::operator fp32x2_t() const
{
#if TL_FP4_CVT_DEVICE
...
TL_HOST_DEVICE constexpr pk_fp4_t::operator fp16_t() const
{
#if TL_FP4_CVT_DEVICE
...
TL_HOST_DEVICE constexpr pk_fp4_t::operator fp16x2_t() const
{
#if TL_FP4_CVT_DEVICE
