...
/*
 * GemmMlsTensorOpAB: GEMM with both inputs from MLS LDS, loaded via ds_read_format_tensor.
 *
 * Template params:
 ...
 *   HcuArch: HCU architecture (default gfx938)
 *
                              ...
                              C_type* C_local)
{
    GemmMlsTensorOpAB<M,
                     ...
                     C_type,
                     AccDataType,
                     HcuArch>::body(A_lds, B_lds, C_local);
}

/*
 * GemmMlsTensorOpB: GEMM with B from MLS LDS. A can be from register or swizzle LDS.
 *
                                  ...
                                  C_type* C_local)
    {
        ...
        for (int ki = 0; ki < inner_k; ki++) {
            for (int i = 0; i < warp_rows; i++) {
                ...
                for (int local_id = 0; local_id < (kPack * local_size_a);
                     local_id++) {
                    int row, col;
                    if constexpr (TransposeA) {
                        auto p =
                            GemmOp::reverse_index_map_transposed(lane_id,
                                                                 local_id);
                        row = p.first;
                        col = p.second;
                        A_local[(ki * warp_rows + i) * vec_size + local_id] =
                            A_shared[GemmOp::template make_swizzle_layout<last_dim_a,
                                                                          sizeof(A_type)>(
                                r + row, l + col)];
                    } else {
