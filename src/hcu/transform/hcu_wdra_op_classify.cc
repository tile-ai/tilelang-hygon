...
bool IsHcuWdraVgprCall(const CallNode *call) {
  ...
  if (call->op.same_as(builtin::call_extern())) {
    if (const auto *name = call->args[0].as<StringImmNode>()) {
      ...
      if (value.find("tl::mls::") == 0 || value.find("Atomic") == 0) {
        return true;
      }
    }
  }
...
bool IsHcuWdraDeclarativeStmt(const Stmt &stmt) {
  // Temporary: allow pure alloc / decl_buffer in WDRA prologue (tirx may use
  // data=buf.data alias forms that embed BufferLoad in DeclBuffer).
  if (stmt->IsInstance<AllocBufferNode>() ||
      stmt->IsInstance<DeclBufferNode>()) {
    return true;
  }
  if (const auto *attr = stmt.as<AttrStmtNode>()) {
    if (attr->attr_key == tirx::attr::thread_extent ||
        attr->attr_key == tirx::attr::device_scope) {
      return true;
    }
  }
  return false;
