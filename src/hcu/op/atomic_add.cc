/*!
 * \file hcu/op/atomic_add.cc
 * \brief HCU implementation for tl.atomic_add lowering.
 */
...
bool MatchHCUAtomicAddTarget(Target target) { return TargetIsHCU(target); }

bool RegisterHCUAtomicAdd() {
  RegisterAtomicAddImpl(AtomicAddImpl{
      "hcu.AtomicAdd",
      MatchHCUAtomicAddTarget,
      hcu::AtomicAdd::InferLayout,
      hcu::AtomicAdd::Lower,
  });
  return true;
}

const bool hcu_atomic_add_registered = RegisterHCUAtomicAdd();
