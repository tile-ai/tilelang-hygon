 ...
 * `matrix_load` / `ds_read_format` trans and MLS tile metadata via op args or
 * annotations instead of relying on GEMM pairing here.
 */
inline bool IsGemmTileOpCall(const CallNode *call) {
 ...
 *
 * GEMM consumer/producer pairing (`Find*GemmProducer*`, `has_gemm_consumer_`)
 * matches `tl.tileop.gemm` only (pre-lower `T.gemm` path).
 */
