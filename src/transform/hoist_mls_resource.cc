...
std::string MlsBaseTemplateFromLoadTile(const std::string &sym) {
  ...
  auto args =
      SplitTopLevelTemplateArgs(sym.substr(std::strlen(kMlsLoadTilePrefix),
...
std::optional<std::pair<int64_t, int64_t>> MlsBlockSizesFromLoadTile(const std::string &sym) {
  auto args =
      SplitTopLevelTemplateArgs(sym.substr(std::strlen(kMlsLoadTilePrefix),
                                           sym.size() - std::strlen(kMlsLoadTilePrefix) - 1));
  if (args.empty()) {
  ...
  auto dims = SplitTopLevelTemplateArgs(block_size.substr(begin + 1, end - begin - 1));
  if (dims.size() < 2) {
    return std::nullopt;
  }
  try {
...
bool ExprUsesAnyVar(const PrimExpr &expr, const std::unordered_set<const VarNode *> &vars) {
  bool found = false;
  tir::PostOrderVisit(expr, [&](const ObjectRef &node) {
    if (found) return;
    if (const auto *var = node.as<VarNode>()) {
...
void CollectForLoopVars(const Stmt &stmt, std::unordered_set<const VarNode *> *vars) {
...
void CollectMlsLoadTileCalls(const Stmt &stmt, std::vector<const CallNode *> *calls) {
  tir::PostOrderVisit(stmt, [&](const ObjectRef &node) {
...
std::string ResourceKey(const std::string &base_template, const CallNode *call) {
  std::ostringstream os;
  os << base_template;
  for (int i = 1; i <= 5; ++i) {
...
Stmt MakeExternStmt(const std::string &symbol, const std::vector<PrimExpr> &args) {
...
class HoistMlsResourceMutator : public StmtMutator {
 public:
  Stmt VisitStmt_(const SeqStmtNode *op) final {
    ...
    ++scope_depth_;
    PredeclareMlsResources(tvm::ffi::GetRef<Stmt>(op), /*loop_var=*/nullptr, &prelude,
                           &nested_loop_vars);
  ...
  Stmt VisitStmt_(const ForNode *op) final {
    ...
    ++scope_depth_;
    active_loop_ranges_.push_back({op->loop_var, Range::FromMinExtent(op->min, op->extent)});
    Stmt new_body = VisitStmt(op->body);
    active_loop_ranges_.pop_back();

    Stmt loop = For(op->loop_var, op->min, op->extent, op->kind,
                    std::move(new_body), op->thread_binding, op->annotations,
                    op->step, op->span);
  ...
  Stmt VisitStmt_(const EvaluateNode *op) final {
    ...
    if (update_mn_scopes_.count(key)) {
      ...
      if (auto block_sizes = MlsBlockSizesFromLoadTile(sym)) {
        check_mn_filter =
            !CanProveNoBoundary(call->args[5], call->args[3], block_sizes->first);
        check_k_filter =
            !CanProveNoBoundary(call->args[6], call->args[4], block_sizes->second);
      }
      ...
      seq.push_back(MakeExternStmt(async_sym.str(), {StringImm(obj_name), call->args[7], call->args[5]}));
    } else {
      seq.push_back(MakeExternStmt("tl::mls::update_base", {StringImm(obj_name), call->args[6]}));
      const std::string data_type = MlsDataTypeFromLoadTile(sym);
      auto [check_last_load, last_load] = MlsLastLoadTemplateArgs(sym);
      if (auto block_sizes = MlsBlockSizesFromLoadTile(sym)) {
        if (CanProveNoBoundary(call->args[6], call->args[4], block_sizes->second)) {
      ...
      seq.push_back(MakeExternStmt(async_sym.str(), {StringImm(obj_name), call->args[7], call->args[6]}));
    }
    return SeqStmt(seq);
  }
  ...
  void PredeclareMlsResources(
    ...
    for (const CallNode *call : calls) {
      ICHECK_EQ(call->args.size(), 8U)
          << "mls_load_tile extern expects symbol, src, stride, mn_len, k_len, mn_base, k_base, dst";
      if ((loop_var != nullptr &&
           (ExprUsesVar(call->args[1], loop_var) || ExprUsesVar(call->args[2], loop_var) ||
            ExprUsesVar(call->args[3], loop_var) || ExprUsesVar(call->args[4], loop_var))) ||
      ...
      const std::string obj_name = "_tl_mls_h_" + std::to_string(resource_counter_++);
      resource_names_[key] = obj_name;
      resource_scopes_[key] = scope_depth_;
      prelude->push_back(MakeExternStmt(kMlsResourceInitPrefix + base_template.substr(0) + ">",
                                        {StringImm(obj_name), call->args[1], call->args[2],
                                         call->args[3], call->args[4]}));
    }
  }

  void PreinitializeMlsWindows(const Stmt &body, const VarNode *loop_var, std::vector<Stmt> *prelude) {
    std::vector<const CallNode *> calls;
    CollectMlsLoadTileCalls(body, &calls);
    ...
    for (const CallNode *call : calls) {
      const auto *sym_node = call->args[0].as<StringImmNode>();
      const std::string base_template = MlsBaseTemplateFromLoadTile(sym_node->value);
      const std::string key = ResourceKey(base_template, call);
    ...
    for (const CallNode *call : calls) {
      ...
      if (!initialized_scopes_.count(key)) {
        PrimExpr init_mn = mn_depends_on_loop ? PrimExpr(IntImm(DataType::Int(32), 0)) : call->args[5];
        PrimExpr init_k = mn_depends_on_loop ? call->args[6] : PrimExpr(IntImm(DataType::Int(32), 0));
      ...
      if (update_mn_scopes_.count(key) && !mn_depends_on_loop &&
        ...
        prelude->push_back(MakeExternStmt("tl::mls::update_mn_base",
                                          {StringImm(it->second), call->args[5]}));
        updated_mn_scopes_[key] = scope_depth_;
      }
    }
  }

  bool CanProveNoBoundary(const PrimExpr &base, const PrimExpr &length, int64_t block_size) const {
    if (block_size <= 0) {
      return false;
    }
    ...
    PrimExpr block = tir::make_const(base.dtype(), block_size);
...
tvm::transform::Pass HoistMlsResource() {
  ...
  return tir::transform::CreatePrimFuncPass(pass_func, 0, "tl.HoistMlsResource", {});
}
