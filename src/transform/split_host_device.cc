  ...
  tirx::Stmt SplitDeviceFunc(tirx::Stmt body, tvm::Target device_target) {
    ...
    auto [old_params, buffers_to_declare] =
      ...
      // that only use a dynamic extent in buffer metadata / assumes).
      {
        ...
        auto push_from_expr = [&](const PrimExpr &e) {
          Array<tirx::Var> defined;
          defined.reserve(params.size());
          for (const auto &v : params) {
            defined.push_back(v);
          }
          for (const tirx::Var &v : tirx::UndefinedVars(e, defined)) {
            if (seen.insert(v.get()).second) {
              params.push_back(v);
            }
          }
        };
        for (const tirx::Buffer &buf : use_def.undefined_buffers_) {
          for (const PrimExpr &dim : buf->shape) {
            push_from_expr(dim);
          }
          for (const PrimExpr &stride : buf->strides) {
            push_from_expr(stride);
          }
          push_from_expr(buf->elem_offset);
        }
      }
      SortDeviceParams(&params);
