 ...
 * shared Buffer + Region ranges (like cp.async pipeline planning), not on merged
 * dyn-shmem byte offsets.
 *
...
bool AnnotationIsTrue(const Map<String, ObjectRef> &annotations, const char *key) {
...
public:
  ...
  void Plan() {
    if (!has_mls_tile_ops_) return;
    for (const TimelineEvent &ev : timeline_) {
      if (ev.kind != EventKind::kConsumer) continue;
  ...
  std::unordered_map<const Object *, std::vector<WaitPlan>> OccurrencePlans() const {
    std::unordered_map<const Object *, std::vector<WaitPlan>> out;
    for (const TimelineEvent &ev : timeline_) {
      if (ev.kind != EventKind::kConsumer) continue;
      auto it = plan_by_index_.find(ev.index);
      if (it == plan_by_index_.end()) continue;
...
private:
  ...
  int CountOutstandingSincePrevConsumer(int consumer_idx, const LoopCtx *loop) const {
    // Interleaved pipeline (ds_read A -> reload A -> ds_read B): vmcnt is global,
    // so count MLS producers issued after the previous consumer in this loop body.
    int prev_consumer = -1;
    ...
    if (prev_consumer < 0) return 0;
    if (loop != nullptr && prev_consumer < loop->begin) return 0;

    int keep = 0;
    for (int i = prev_consumer + 1; i < consumer_idx; ++i) {
      if (timeline_[i].kind != EventKind::kProducer) continue;
      if (timeline_[i].issue_count <= 0) return 0;
      keep += timeline_[i].issue_count;
    }
    return keep;
  }

  bool ConsumerInsideLoop(const TimelineEvent &consumer, const LoopCtx *loop) const {
    if (loop == nullptr) return false;
    return consumer.index >= loop->begin && consumer.index < loop->end;
  }

  int FinalizeKeep(int keep, const TimelineEvent &consumer, const LoopCtx *loop) const {
    if (ConsumerInsideLoop(consumer, loop)) {
      keep = std::max(keep, CountOutstandingSincePrevConsumer(consumer.index, loop));
    }
    if (keep > 63) return 0;
    return keep;
  }
  ...
  std::optional<WaitPlan> PlanConsumer(const TimelineEvent &consumer) {
    ...
    for (const SharedRegion &creg : consumer.regions) {
      ...
      plan.vmcnt = std::min(plan.vmcnt, FinalizeKeep(match->keep, consumer, loop));
      plan.producer_indices.insert(match->producer_idx);
    }
    if (plan.vmcnt == std::numeric_limits<int>::max()) plan.vmcnt = 0;
    return plan;
  }

  std::optional<MatchResult> MatchProducerAcrossPhases(const TimelineEvent &consumer,
                                                       const SharedRegion &creg,
                                                       const LoopCtx *loop) {
    // One wait is shared by every loop phase at runtime. Take the largest keep so
    // the inserted vmcnt reflects the true outstanding MLS count (phase 0 prologue
    // pairing), not the tighter carry-over from later phases.
    ...
    for (int64_t phase = 0; phase < loop->extent; ++phase) {
      if (auto m = FindProducer(consumer.index, creg, loop, phase, false)) {
        if (!best || m->keep > best->keep) best = m;
      }
      if (phase > 0) {
  ...
  int CountOutstandingEpilogue(int prod_idx, int consumer_idx, const LoopCtx *loop,
                               int64_t prod_phase) {
  ...
  std::optional<MatchResult> MatchEpilogueProducer(const TimelineEvent &consumer,
                                                   const SharedRegion &creg,
                                                   const LoopCtx *loop) {
    auto matches = [&](const TimelineEvent &prod, int64_t prod_phase) {
      for (const SharedRegion &preg : prod.regions) {
        if (RegionMatchAtPhase(preg, creg, loop, prod_phase, 0)) return true;
      }
      return false;
    };

    // Prefer the latest loop iteration that still writes this slot (k = last, last-1, ...).
    for (int64_t phase = loop->extent - 1; phase >= 0; --phase) {
      for (int i = loop->end - 1; i >= loop->begin; --i) {
        if (timeline_[i].kind != EventKind::kProducer) continue;
        if (!matches(timeline_[i], phase)) continue;
        return MatchResult{i, CountOutstandingEpilogue(i, consumer.index, loop, phase)};
      }
    }
    return std::nullopt;
  }

  std::optional<MatchResult> MatchProducerForRegion(const TimelineEvent &consumer,
                                                    const SharedRegion &creg,
                                                    const LoopCtx *loop) {
    const bool inside_loop =
        consumer.loop_id >= 0 && loop != nullptr &&
    ...
    // Epilogue: constant shared regions must pair with the last loop writer, not prologue.
    if (loop != nullptr && loop->extent > 1) {
      if (auto m = MatchEpilogueProducer(consumer, creg, loop)) return m;
    }

    return FindProducer(consumer.index, creg, nullptr, std::nullopt, false);
  }

  std::optional<MatchResult> FindProducer(int consumer_idx, const SharedRegion &creg,
                                          const LoopCtx *loop, std::optional<int64_t> phase,
                                          bool wrap) {
    auto try_producer = [&](int prod_idx, int keep) -> MatchResult {
      return MatchResult{prod_idx, keep};
    };

    auto matches = [&](const TimelineEvent &prod, int64_t prod_phase, int64_t cons_phase) {
      for (const SharedRegion &preg : prod.regions) {
        if (RegionMatchAtPhase(preg, creg, loop, prod_phase, cons_phase)) return true;
      }
      return false;
    };
  ...
  bool RegionMatchConcrete(const std::vector<SharedRegion> &prods, const SharedRegion &cons) {
    for (const SharedRegion &preg : prods) {
      if (RegionMatchAtPhase(preg, cons, nullptr, 0, 0)) return true;
    }
    return false;
  }

  PrimExpr EvalExprPhase(const PrimExpr &expr, const LoopCtx *loop, int64_t phase) {
    if (loop == nullptr) return analyzer_.Simplify(expr);
  ...
  bool ExprEqual(const PrimExpr &lhs, const PrimExpr &rhs) {
    ...
    if (const int64_t *ai = as_const_int(a)) {
      if (const int64_t *bi = as_const_int(b)) return *ai == *bi;
    }
    return analyzer_.CanProve(analyzer_.Simplify(a == b), arith::ProofStrength::kSymbolicBound);
  }

  bool RangeEqualAtPhase(const Range &lhs, const Range &rhs, const LoopCtx *loop,
                         int64_t lhs_phase, int64_t rhs_phase) {
  ...
  bool RegionMatchAtPhase(const SharedRegion &prod, const SharedRegion &cons, const LoopCtx *loop,
                          int64_t prod_phase, int64_t cons_phase) {
    if (!prod.buffer.same_as(cons.buffer)) return false;
    if (prod.ranges.size() != cons.ranges.size()) return false;
    for (size_t i = 0; i < prod.ranges.size(); ++i) {
      if (!RangeEqualAtPhase(prod.ranges[i], cons.ranges[i], loop, prod_phase, cons_phase)) {
        return false;
      }
    }
    return true;
  ...
  bool ConsumerRegionsOverlap(const TimelineEvent &lhs, const TimelineEvent &rhs) {
    for (const SharedRegion &lreg : lhs.regions) {
      for (const SharedRegion &rreg : rhs.regions) {
        if (RegionMatchAtPhase(lreg, rreg, nullptr, 0, 0)) return true;
      }
    }
    return false;
  }
  ...
  int CountOutstandingLoopCarry(int prod_idx, int consumer_idx, const LoopCtx *loop) {
    int keep = CountOutstanding(prod_idx, loop->end);
  ...
  void MergePlans() {
    ...
    for (auto it = consumer_idxs.rbegin(); it != consumer_idxs.rend(); ++it) {
      ...
      if (cur == plan_by_index_.end()) continue;
      int cur_vmcnt = cur->second.vmcnt;
      bool drop = false;
      for (int i = *it - 1; i >= 0; --i) {
        const TimelineEvent &prev = timeline_[i];
        if (prev.kind == EventKind::kProducer) {
          if (cur->second.producer_indices.count(i)) break;
          continue;
        }
        if (prev.kind != EventKind::kConsumer) continue;
        auto prev_plan = plan_by_index_.find(i);
        if (prev_plan != plan_by_index_.end() && prev_plan->second.vmcnt <= cur_vmcnt &&
            ConsumerRegionsOverlap(prev, cons)) {
          drop = true;
          break;
        }
      }
      if (drop) erase.push_back(*it);
    }
    for (int idx : erase) plan_by_index_.erase(idx);
  }

  const LoopCtx *LoopForConsumer(const TimelineEvent &consumer) {
    if (consumer.loop_id >= 0 && consumer.loop_id < static_cast<int>(loops_.size())) {
      return &loops_[consumer.loop_id];
    }
    for (auto it = loops_.rbegin(); it != loops_.rend(); ++it) {
      if (it->extent > 1) return &(*it);
    }
  ...
  void VisitStmt_(const ForNode *op) final {
    ...
    int64_t ext = 1;
    if (const auto *imm = op->extent.as<IntImmNode>()) ext = imm->value;
    loops_.push_back(LoopCtx{op->loop_var, op->loop_var.get(), static_cast<int>(timeline_.size()),
                             0, ext, GetPipelineNumStages(op).defined()});
  ...
  void VisitStmt_(const EvaluateNode *op) final {
    if (const auto *call = op->value.as<CallNode>()) {
      ...
      if (tir_op == MatrixLoad::Get()) {
        ...
        const bool a_from_mls = AnnotationIsTrue(call->annotations, attr::kHcuAFromMls);
        const bool b_from_mls = AnnotationIsTrue(call->annotations, attr::kHcuBFromMls);
        std::vector<SharedRegion> consumer_regions;
        if (a_from_mls && IsSharedBuffer(gemm->a_)) {
          consumer_regions.push_back(SharedRegionFromBufferRegion(gemm->aRegion_));
        }
        if (b_from_mls && IsSharedBuffer(gemm->b_)) {
          consumer_regions.push_back(SharedRegionFromBufferRegion(gemm->bRegion_));
        }
...
public:
  explicit InsertWaitcntMutator(
      std::unordered_map<const Object *, std::vector<WaitPlan>> occurrence_plans)
      : occurrence_plans_(std::move(occurrence_plans)) {}

  Stmt VisitStmt_(const EvaluateNode *op) final {
    Stmt body = StmtMutator::VisitStmt_(op);
    auto it = occurrence_plans_.find(op);
    if (it == occurrence_plans_.end() || it->second.empty()) return body;
...
private:
  static int PackVmcntImm(int vmcnt) {
    ...
    return (vmcnt & 0xF) | (kExpcnt << 4) | (kUbResearch << 7) | (kLgkmcnt << 8) |
           (kBits1213 << 12) | ((vmcnt & 0x30) << 10);
  }
  ...
  static Stmt MakeSync() {
    return Evaluate(Call(DataType::Int(32), builtin::tvm_storage_sync(), {StringImm("shared")}));
  }
...
PrimFunc InsertMlsWaitcnt(PrimFunc func) {
  ...
  if (!planner.HasMlsTileOps()) return func;
  planner.Plan();
  if (!planner.HasPlans()) return func;

  auto *n = func.CopyOnWrite();
  n->body = InsertWaitcntMutator(planner.OccurrencePlans())(std::move(func->body));
  return func;
}
...
tvm::transform::Pass InsertMlsWaitcntPass() {
  auto pass_func = [](PrimFunc f, const IRModule &m, const PassContext &ctx) {
    bool disable = ctx->GetConfig(kDisableThreadStorageSync, Bool(false)).value()->value;
    if (disable) return f;
    return tl::InsertMlsWaitcnt(std::move(f));
  };
  return tir::transform::CreatePrimFuncPass(pass_func, 0, "tl.InsertMlsWaitcnt", {});
}
