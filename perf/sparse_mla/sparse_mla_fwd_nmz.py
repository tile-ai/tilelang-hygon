...
def sparse_mla_fwd(
    ...
    *,
    ...
    glse_shape = [batch_head, seq_len, num_heads, num_split]
    output_partial_shape = [batch_head, seq_len, num_heads, num_split, dim]
    if num_split_tail > 0:
        glse_shape_tail = [batch_tail, seq_len, num_heads, num_split_tail]
        output_partial_shape_tail = [batch_tail, seq_len, num_heads, num_split_tail, dim]
    ...
    if num_split_tail > 0:
        ...
        @T.macro
        def combine_tail(
            glse_tail: T.Tensor(glse_shape_tail, dtype),
            Output_partial_tail: T.Tensor(output_partial_shape_tail, dtype),
            ...
            with T.Kernel(seq_len, num_heads, batch_tail, threads=128) as (bx, by, bz):
                ...
                for k in T.serial(num_split_tail):
                    lse_max_local[0] = T.max(lse_max_local[0], glse_tail[bz, bx, by, k])
                for k in T.Pipelined(num_split_tail, num_stages=1):
                    lse_local_split[0] = glse_tail[bz, bx, by, k]
                    lse_logsum_local[0] += T.exp2(lse_local_split[0] - lse_max_local[0])
                lse_logsum_local[0] = T.log2(lse_logsum_local[0]) + lse_max_local[0]
                for k in T.serial(num_split_tail):
                    for i in T.Parallel(dim):
                        po_local[i] = Output_partial_tail[bz, bx, by, k, i]
                    lse_local_split[0] = glse_tail[bz, bx, by, k]
                    scale_local[0] = T.exp2(lse_local_split[0] - lse_logsum_local[0])
                    for i in T.Parallel(dim):
                        o_accum_local[i] += po_local[i] * scale_local[0]
        ...
        @T.macro
        def combine_all(
            glse: T.Tensor(glse_shape, dtype),
            Output_partial: T.Tensor(output_partial_shape, dtype),
            glse_tail: T.Tensor(glse_shape_tail, dtype),
            Output_partial_tail: T.Tensor(output_partial_shape_tail, dtype),
            Output: T.Tensor(o_shape, dtype),
        ):
            with T.Kernel(seq_len, num_heads, batch, threads=128) as (bx, by, bz):
                ...
                if bz < batch_head:
                    ...
                    lse_max_local[0] = -T.infinity(accum_dtype)
                    for k in T.serial(num_split_tail):
                        lse_max_local[0] = T.max(lse_max_local[0], glse_tail[bz_tail, bx, by, k])
                    for k in T.Pipelined(num_split_tail, num_stages=1):
                        lse_local_split[0] = glse_tail[bz_tail, bx, by, k]
                        lse_logsum_local[0] += T.exp2(lse_local_split[0] - lse_max_local[0])
                    lse_logsum_local[0] = T.log2(lse_logsum_local[0]) + lse_max_local[0]
                    for k in T.serial(num_split_tail):
                        for i in T.Parallel(dim):
                            po_local[i] = Output_partial_tail[bz_tail, bx, by, k, i]
                        lse_local_split[0] = glse_tail[bz_tail, bx, by, k]
                        scale_local[0] = T.exp2(lse_local_split[0] - lse_logsum_local[0])
                        for i in T.Parallel(dim):
                            o_accum_local[i] += po_local[i] * scale_local[0]
...
def sparse_mla_fwd_interface(q,
    ...
    if batch_head is None:
        batch_head = batch
    assert num_split > 0, "num_split must be greater than 0"
