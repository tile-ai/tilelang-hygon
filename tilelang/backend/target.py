...
# Prefer HCU over generic HIP when both detectors match the same gfx arch.
_TARGET_DETECTOR_PRIORITY: tuple[str, ...] = ("cuda", "hcu", "hip", "metal", "llvm", "c", "webgpu")
...
def auto_detect_target() -> TargetInput:
    ...
    def try_detector(spec: TargetDetectorSpec) -> TargetInput | None:
        try:
    ...
    for name in _TARGET_DETECTOR_PRIORITY:
        spec = _TARGET_DETECTORS.get(name)
        if spec is None:
            continue
        seen.add(name)
        detected = try_detector(spec)
