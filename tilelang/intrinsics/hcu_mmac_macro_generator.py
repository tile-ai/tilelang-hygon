...
from tilelang.intrinsics.hcu_mmac_emitter_utils import (
...
from tvm.tir import Buffer, BufferLoad, BufferRegion, PrimExpr, Var
...
class HCUMatrixCoreIntrinEmitter:
    ...
    def _initialize_block_warp_tiles(self, block_m: int, block_n: int) -> None:
        ...
        self.block_col_warps_no_recompute = block_col_warps_no_recompute(
            self.block_n, self.block_col_warps, self.min_n_per_warp
        )
    ...
    def _initialize_is_m_first(self, is_m_first: bool | None = False):
        ...
        if is_m_first:
            raise ValueError(
                "HCU MMAC emitter: is_m_first=True is not supported; "
                "warp order is fixed M->N->K to match layout inference"
            )
    ...
    def ldmatrix_mls_a(self, A_local_buf, A_mls_src: Buffer | BufferLoad | BufferRegion):
        ...
        tile_mn, tile_k = compute_mls_tiles(
            mls_trans, block_mn, block_k, self.threads, self.target, elem_bits(A_local_buf.dtype)
        )
    ...
    def ldmatrix_mls_b(self, B_local_buf, B_mls_src: Buffer | BufferLoad | BufferRegion):
        ...
        tile_mn, tile_k = compute_mls_tiles(
            mls_trans, block_mn, block_k, self.threads, self.target, elem_bits(B_local_buf.dtype)
        )
    ...
    def mmac(self, A_local_buf, B_local_buf, C_local_buf, k_inner: PrimExpr | int = 0):
        ...
        mmac_op = tir.op.Op.get("tl.tvm_mfma")
